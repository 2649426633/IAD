...
namespace IAD.Pages
{
    public class OnlineInspectionPage : UserControl
    {
        ...
        private void Build()
        {
            ...
            root.RowStyles.Add(new RowStyle(SizeType.Absolute, 54F)); root.RowStyles.Add(new RowStyle(SizeType.Percent, 100F)); root.RowStyles.Add(new RowStyle(SizeType.Absolute, 155F));
            FlowLayoutPanel toolbar = UiFactory.Toolbar("加载图片", "批量检测", "开始检测", "暂停", "导出结果");
            Label metrics = UiFactory.Label("当前速度  28件/分     今日检测  12,560件     NG率  2.34%", 10F, true, ContentAlignment.MiddleRight); metrics.Width = 460; metrics.Height = 35; toolbar.Controls.Add(metrics); root.Controls.Add(toolbar, 0, 0);
            TableLayoutPanel body = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 3, RowCount = 1, BackColor = UiTheme.Page };
            body.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 235F)); body.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100F)); body.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 305F));
            ...
            {
                int gap = 18; int w = (canvas.Width - gap * 5) / 4; int h = (canvas.Height - gap * 3) / 2;
                for (int i = 0; i < 8; i++)
                {
                    int r = i / 4, c = i % 4; Rectangle box = new Rectangle(gap + c * (w + gap), gap + r * (h + gap), w, h);
                    ...
                    if (state == "NG") using (Pen p = new Pen(Color.White, 2)) e.Graphics.DrawLine(p, box.X + 20, box.Bottom - 35, box.Right - 18, box.Y + 30);
                }
            };
            ...
            right.Controls.Add(UiFactory.Card("#2 缺陷详情", UiFactory.Grid(new[] { "类型", "置信度", "面积(px)", "位置" }, new[] { new[] { "划痕", "0.96", "1824", "(1320,182)" } })), 0, 1); body.Controls.Add(right, 2, 0); root.Controls.Add(body, 0, 1);
            TableLayoutPanel bottom = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 3, RowCount = 1, BackColor = UiTheme.Page };
            bottom.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 48F)); bottom.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 24F)); bottom.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 28F));
            ...
            bottom.Controls.Add(UiFactory.Card("NG原因统计（今日）", UiFactory.Grid(new[] { "缺陷", "数量", "占比" }, new[] { new[] { "划痕", "186", "45.8%" }, new[] { "脏污", "128", "31.5%" }, new[] { "缺口", "73", "18.0%" }, new[] { "变形", "19", "4.7%" } })), 2, 0); root.Controls.Add(bottom, 0, 2); Controls.Add(root);
        }
    }
}
