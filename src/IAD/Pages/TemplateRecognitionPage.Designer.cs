...
namespace IAD.Pages
{
    public partial class TemplateRecognitionPage
    {
        private void InitializeComponent()
        {
            ...
            left.Controls.Add(UiFactory.Card("Hard Negative", UiFactory.Grid(new[] { "ID", "来源", "相似度" }, hn)), 0, 1);
            ...
            query.Paint += delegate(object sender, PaintEventArgs e)
            {
                using (Pen p = new Pen(Color.Black, 2)) e.Graphics.DrawLine(p, 50, query.Height - 45, query.Width - 55, 45);
            ...
            heat.Paint += delegate(object sender, PaintEventArgs e)
            {
                using (Brush b = new SolidBrush(Color.FromArgb(80, 80, 80))) e.Graphics.FillEllipse(b, heat.Width / 3, heat.Height / 4, heat.Width / 3, heat.Height / 2);
                e.Graphics.DrawString("相似度热力图", UiTheme.Font(9F, true), Brushes.Black, 12, 12);
            };
            analysis.Controls.Add(UiFactory.Card("查询瑕疵（待标注）", query), 0, 0);
            analysis.Controls.Add(UiFactory.Card("相似度热力图", heat), 1, 0);
            ...
            center.Controls.Add(UiFactory.Card("候选区域（Top-K）", UiFactory.Grid(new[] { "排名", "相似度", "来源图像", "状态" }, candidates)), 0, 1);
            ...
            {
                Label l = UiFactory.Label(steps[i], 9F, i % 2 == 0, ContentAlignment.MiddleCenter);
                l.Width = i % 2 == 0 ? 120 : 28;
                l.Height = 45;
                loop.Controls.Add(l);
            }
            root.Controls.Add(UiFactory.Card("学习循环进度  |  本轮：已确认126 / 已拒绝34 / 待处理28", loop), 0, 2);
            Controls.Add(root);
        }
    }
}
