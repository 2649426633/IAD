...
namespace IAD.Pages
{
    public partial class DashboardPage
    {
        ...
        private Control BuildProcess()
        {
            ...
            for (int i = 0; i < names.Length; i++)
            {
                ...
                step.Paint += delegate(object sender, PaintEventArgs e)
                {
                    ...
                    {
                        if (pending) pen.DashStyle = System.Drawing.Drawing2D.DashStyle.Dash;
                        e.Graphics.DrawRectangle(pen, 0, 0, p.Width - 1, 66);
                    }
                };
                Label number = UiFactory.Label((i + 1).ToString(), 10.5F, false, ContentAlignment.MiddleCenter);
                number.Height = 25;
                number.Dock = DockStyle.Top;
                number.ForeColor = pending ? UiTheme.Muted : UiTheme.Text;
                Label name = UiFactory.Label(names[i], 8.8F, active, ContentAlignment.MiddleCenter);
                name.Height = 41;
                name.Dock = DockStyle.Top;
                name.ForeColor = pending ? UiTheme.Muted : UiTheme.Text;
                TableLayoutPanel stateArea = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 1, RowCount = 2, BackColor = Color.Transparent, Margin = Padding.Empty, Padding = Padding.Empty };
                stateArea.RowStyles.Add(new RowStyle(SizeType.Percent, 52F));
                stateArea.RowStyles.Add(new RowStyle(SizeType.Percent, 48F));
                Label state = UiFactory.Label(states[i], 8.2F, active, ContentAlignment.BottomCenter);
                state.ForeColor = pending ? UiTheme.Muted : UiTheme.Text;
                Label date = UiFactory.Label(dates[i], 7.5F, false, ContentAlignment.TopCenter);
        ...
        {
            TableLayoutPanel stats = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 5, RowCount = 1, BackColor = UiTheme.Page, Margin = Padding.Empty, Padding = Padding.Empty };
        ...
        {
            TableLayoutPanel bottom = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 3, RowCount = 1, BackColor = UiTheme.Page, Margin = Padding.Empty, Padding = Padding.Empty };
