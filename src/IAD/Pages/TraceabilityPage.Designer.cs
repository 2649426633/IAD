...
namespace IAD.Pages
{
    public partial class TraceabilityPage
    {
        private void InitializeComponent()
        {
            ...
            {
                int size = System.Math.Min(preview.Width, preview.Height) - 70;
                Rectangle r = new Rectangle((preview.Width - size) / 2, (preview.Height - size) / 2, size, size);
                using (Pen p = new Pen(Color.White, 2)) e.Graphics.DrawEllipse(p, r);
                using (Pen p = new Pen(Color.Black, 3))
                {
                    e.Graphics.DrawRectangle(p, r.X + 25, r.Y + 30, 95, 60);
                    e.Graphics.DrawRectangle(p, r.Right - 125, r.Y + 100, 90, 65);
                }
                e.Graphics.DrawString("NG_01 划痕 0.92", UiTheme.Font(8F, true), Brushes.Black, r.X + 28, r.Y + 34);
                e.Graphics.DrawString("NG_02 缺口 0.87", UiTheme.Font(8F, true), Brushes.Black, r.Right - 120, r.Y + 104);
            };
            body.Controls.Add(UiFactory.Card("选中记录预览", preview), 1, 0);
            body.Controls.Add(UiFactory.Card("检测详情", UiFactory.KeyValues(new[,] { { "原图：", "IMG_20250516_143018.png" }, { "产品实例：", "21 / 24" }, { "Mask：", "mask_..._21.png" }, { "缺陷实例：", "2 个" }, { "类别：", "划伤、缺口" }, { "概率：", "0.92、0.87" }, { "最终状态：", "NG" }, { "Recipe：", "V2.1.0" }, { "模型版本：", "Model_A_2.1.2" }, { "操作员：", "operator_01" } }, 42)), 2, 0);
