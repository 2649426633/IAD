...
namespace IAD.Pages
{
    public partial class ProductDefinitionPage
    {
        ...
        private Control BuildTemplateWorkspace()
        {
            ...
            for (int i = 0; i < 4; i++) footer.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 25F));
        ...
        private void PaintTemplateCanvas(object sender, PaintEventArgs e)
        {
            ...
            using (Pen productPen = new Pen(Color.FromArgb(215, 215, 215), 2F))
            {
                ...
                using (SolidBrush innerBrush = new SolidBrush(Color.FromArgb(112, 112, 112))) e.Graphics.FillRectangle(innerBrush, inner);
                int hole = 18;
            ...
            using (SolidBrush brush = new SolidBrush(Color.White)) e.Graphics.DrawString("Template ROI", font, brush, roi.X + 8, roi.Y + 7);
            using (Font tipFont = UiTheme.Font(8.2F, false))
            using (SolidBrush tipBrush = new SolidBrush(Color.FromArgb(205, 205, 205))) e.Graphics.DrawString("基准图预览 · 鼠标框选产品区域作为定位模板", tipFont, tipBrush, 14, 12);
        }

        private static void DrawHole(Graphics g, int x, int y, int size)
        {
            using (SolidBrush brush = new SolidBrush(Color.FromArgb(55, 55, 55))) g.FillEllipse(brush, x, y, size, size);
        }
        ...
        {
            TableLayoutPanel stack = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 1, RowCount = 2, BackColor = UiTheme.Page, Margin = Padding.Empty, Padding = Padding.Empty };
        ...
        {
            TableLayoutPanel panel = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 2, RowCount = 7, BackColor = UiTheme.Surface, Margin = Padding.Empty, Padding = new Padding(12, 4, 12, 8) };
            panel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 42F));
            panel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 58F));
            for (int i = 0; i < 7; i++) panel.RowStyles.Add(new RowStyle(SizeType.Percent, 14.285F));
        ...
        {
            TableLayoutPanel panel = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 2, RowCount = 6, BackColor = UiTheme.Surface, Margin = Padding.Empty, Padding = new Padding(12, 4, 12, 8) };
            panel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 42F));
            panel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 58F));
            for (int i = 0; i < 6; i++) panel.RowStyles.Add(new RowStyle(SizeType.Percent, 16.666F));
        ...
        {
            TableLayoutPanel body = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 1, RowCount = 2, BackColor = UiTheme.Surface, Margin = Padding.Empty, Padding = new Padding(10, 0, 10, 8) };
            body.RowStyles.Add(new RowStyle(SizeType.Absolute, 40F));
            body.RowStyles.Add(new RowStyle(SizeType.Percent, 100F));
            FlowLayoutPanel tools = new FlowLayoutPanel { Dock = DockStyle.Fill, FlowDirection = FlowDirection.LeftToRight, WrapContents = false, BackColor = UiTheme.Surface, Margin = Padding.Empty, Padding = new Padding(0, 5, 0, 5) };
            ...
            DataGridView grid = UiFactory.Grid(new[] { "序号", "缺陷名称", "缺陷类型", "检测策略", "默认阈值", "最小面积(px)", "最小长度(px)", "状态" }, defects);
            body.Controls.Add(tools, 0, 0);
