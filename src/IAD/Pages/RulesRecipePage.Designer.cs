...
namespace IAD.Pages
{
    public partial class RulesRecipePage
    {
        private void InitializeComponent()
        {
            ...
            {
                Rectangle all = new Rectangle(35, 25, zones.Width - 70, zones.Height - 60);
                using (Pen p = new Pen(Color.Black, 2)) e.Graphics.DrawRectangle(p, all);
                Rectangle critical = new Rectangle(all.X + 55, all.Y + 30, all.Width / 2, all.Height - 60);
                using (Pen p = new Pen(Color.FromArgb(70, 70, 70), 3)) e.Graphics.DrawRectangle(p, critical);
                Rectangle normal = new Rectangle(all.Right - all.Width / 4 - 20, all.Y + 30, all.Width / 5, all.Height - 60);
            ...
            top.Controls.Add(UiFactory.Card("检测区域定义", zones), 1, 0);
            root.Controls.Add(top, 0, 0);

            TableLayoutPanel mid = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 3, RowCount = 1, BackColor = UiTheme.Page };
            mid.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 38F));
            mid.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 34F));
            mid.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 28F));
            ...
            mid.Controls.Add(UiFactory.Card("Inspection Recipe 版本", UiFactory.Grid(new[] { "版本", "状态", "创建时间", "创建人" }, new[] { new[] { "1.0.0", "当前", "05-16 14:35", "Admin" }, new[] { "0.9.2", "已发布", "05-14 10:21", "Admin" }, new[] { "0.9.1", "已发布", "05-12 16:42", "Admin" }, new[] { "0.9.0", "已发布", "05-10 11:07", "Admin" } })), 2, 0);
            ...
            bottom.Controls.Add(UiFactory.Card("策略预估（验证集）", UiFactory.KeyValues(new[,] { { "预计FNR：", "1.68% / 达标" }, { "预计FPR：", "3.72% / 达标" }, { "综合代价：", "0.86 / 较优" }, { "评估样本：", "18,732" } }, 40)), 1, 0);
